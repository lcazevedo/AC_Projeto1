...
static int parser(char *s, struct PrepNumber *pn)
{
	...
	while (state != FSM_STOP)
	{
		switch (state)
		{
			case FSM_A:    // tira os espaços iniciais
				if (ISSPACE(c)) c = GETC(s);
				else state = FSM_B;
			break;

            // essa verificação tem que ter no meu laço
			case FSM_B:     // define o sinal ou sai do laço se não for um caracter que deveira ser ( +, -, ., ou número)
				state = FSM_C;
			...
			case FSM_D:  // primeira parte parte para tratar a parte decimal.
				...
				{
					c = GETC(s);
					if (pn->exponent > -2147483647) pn->exponent--;  // verifica se o expoente já chegou no mínimo, antes de subtrair 1
				}
				else state = FSM_F;   // e pula para tratr o resto da parte decimal
			break;

            // essa parte tem que ter no meu laço
			case FSM_E:   // trata a parte inteira do número
				if (ISDIGIT(c))
				{
					if (digx < DIGITS)  // digits é o múmero máximo de dígitos que pode ter. digx á a quantidade de dígitos lidos, inicnado com zero
					{
						pn->mantissa *= 10;  // realiza o salto para esquerda de um inteiro da mantissa 
						pn->mantissa += c - '0';   // e coloca o valor do caracter numérico informado
						digx++;
					}
					else if (pn->exponent < 2147483647) pn->exponent++;  // se tem mais número que cabe, aumenta o expoente do 10 em um, "como overffiting?""

					c = GETC(s);
				}
				else if (c == DPOINT)   // quando paparecer ponto, vai para pare de decimal
				{
					c = GETC(s);
					state = FSM_F;
				}
			...
			case FSM_F:
				...
				if (ISDIGIT(c))
				{
					if (digx < DIGITS)  // se ainda está dentro da quantidade de dígito spossíveis...
					{
						pn->mantissa *= 10;   // realiza o salto para esquerda de um inteiro da mantissa
						pn->mantissa += c - '0';  // e coloca o valor do caracter numérico informado
						pn->exponent--;    // e diferente da parte inteira, reduz o expoente em um, incluido um salto da virgula para direita
						digx++;
					}
			...
			case FSM_G:  // tratamento do expoente de 10
				if (c == '+') c = GETC(s);
				else if (c == '-')
				{
					expneg = 1;
			...
			case FSM_H: // tratamento do expoente de 10
				if (c == '0') c = GETC(s);
				else state = FSM_I;
			break;

			case FSM_I: // tratamento do expoente de 10
				if (ISDIGIT(c))
				{
					if (expexp < 214748364)
					{
