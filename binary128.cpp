...
class binary128 {
    ...
    private:
        ...
        static const bitset<113> ZERO;  // para comparação
        ...
        bitset<113> somaBitsets(bitset<113> v1, bitset<113> v2, bool &overflow);
        bitset<113> subtraiBitsets(bitset<113> v1, bitset<113> v2, bool &underflow);
        bitset<113> multiplicaBitsets(bitset<113> v1, bitset<113> v2, int &indice);
        bitset<113> divideBitsets(bitset<113> v1, bitset<113> v2, int &indice);
        
        int getExpoente();
        void setExpoente(int e);
        int getSinal() { return CHKBIT(mf.s_exp, 15);};
        void setSinal(int v) {if (v == 0) CLEARBIT(mf.s_exp, 15); else SETBIT(mf.s_exp, 15);};
...
************/
bitset<113> binary128::somaBitsets(bitset<113> v1, bitset<113> v2, bool &overflow){

    bitset<113> carry, retorno = v1;
	while (v2 != 0) 
	{ 
		carry = retorno & v2;   
        retorno = retorno ^ v2;  
...
************/
bitset<113> binary128::subtraiBitsets(bitset<113> v1, bitset<113> v2, bool  &underflow){
    bitset<114> empresta, n1(v1.to_string()), n2(v2.to_string());
    while (n2 != 0) 
	{ 
		empresta = (~n1 & n2); 
		n1 = n1 ^ n2; 
    ...
    bitset<113> retorno(n1.to_string().substr(1)); // copia eliminado o bit mais significativo
...
************/
bitset<113> binary128::multiplicaBitsets(bitset<113> v1, bitset<113> v2, int &indice) {
    mpz_t n1, n2;
    string retornoStr;
    char *x;
    ...
    bitset<113> resultado(retornoStr.substr(0, 113));

    // se o bit 114 for 1 e o 113 for 0, somo 1 como forma de arredondamento
    if (resultado[112] == 0 && retornoStr[113] == '1')
        resultado[112] == 1;
...
************/
bitset<113> binary128::divideBitsets(bitset<113> v1, bitset<113> v2, int &indice) {
    ...
    string xx;
    int i = 1;
    while (mpz_cmp_ui(resto, 0) != 0 && i < 113)
    {
    ...
    if (retornoStr.length() < 113)
        retornoStr += string(113 - retornoStr.length(),'0');

    // guarda apenas os 113 dígitos da esquerda
    bitset<113> retorno(retornoStr);
...
int binary128::getExpoente(){
    ...
    CLEARBIT(expoente, 15);
    
    // reduz o oofset
    expoente -= EXPOENT_OFFSET;
...
bool binary128::parser(string valor){
    ...
    for (i = 1; i <= 112; i++){ // gera os 112 digitos binarios da mantissa
    ...
    mf.s_exp = expoente + EXPOENT_OFFSET;  // expoente
    if (sinal)
        SETBIT(mf.s_exp, 15); // seta o 16 bit como 1 para indicar negativo (os outros 15 bits são o expoente) 
    for (int i = 0; i < binario.length() && i < 112; i++){
        //cout << "i: " << i << " binario[i] " << binario[i] << endl;
        mf.m[111 - i] = binario[i] - '0';  // guarda com 111 - i para ser da esquera para direita
    }
...
string binary128::to_stringDecimal(void){
    ...
    for (int i = 111; i >= 0; i--) { // percorre os bits da mantissa calculando o valor, com expectativa de até 100 casas decimais
        if (mf.m[i] == 1){
            mpz_pow_ui(temp, v2, (112-i));
...
binary128 binary128::operator+(binary128 b){
    ...
    bitset<113> m1(mf.m.to_string());
    bitset<113> m2(b.mf.m.to_string());
    m1[112] = 1;
    m2[112] = 1;
    bitset<113> zero(string("0"));
    ...
    for (int i = 0; i < 112; i++) retorno.mf.m[i] = m1[i];   
...
binary128 binary128::operator*(binary128 b){
    ...
    //cout << "m1 " << m1 << endl;
    //cout << "m2 " << m2 << endl;
    bitset<113> m = multiplicaBitsets(m1, m2, indiceOverflow);
    //cout << "m " << m << endl;
...
binary128 binary128::operator/(binary128 b){
    ...
    //cout << "m1 " << m1 << endl;
    //cout << "m2 " << m2 << endl;
    bitset<113> m = divideBitsets(m1, m2, indiceAjustado);
    //cout << "m " << m << endl;
